...
export default function UsagePanel() {
  ...
  useEffect(() => {
    ...
    void fetch("/api/usage", { cache: "no-store" })
      .then(async (response) => {
        ...
        const payload = (await response.json()) as { used?: unknown };
  ...
  const status = getUsageLabel(displayedUsed, BETA_AI_RUN_LIMIT);
  const percent = getUsagePercent(displayedUsed, BETA_AI_RUN_LIMIT);
  ...
  return <div>
    ...
    <div className="mt-6 rounded-xl border border-[#f1dfd7] bg-[#fffaf7] p-4 sm:p-5"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-semibold text-[#8d98a9]">Beta guardrail · active and successful AI workflow runs</p><p className="mt-2 text-2xl font-semibold tracking-[-0.04em]">{displayedUsed} <span className="text-sm font-normal text-[#8d98a9]">of {BETA_AI_RUN_LIMIT}</span></p></div><p className="text-xs text-[#8d98a9]">Workspace count · server cap active</p></div><div className="mt-4 h-2 overflow-hidden rounded-full bg-[#f2e4df]"><div className="h-full rounded-full bg-[#d17b54] transition-[width]" style={{ width: `${percent}%` }} /></div><p className="mt-3 text-xs leading-5 text-[#68748a]">The server blocks new active or successful AI runs at the beta limit. This meter is not billing or a claim about account-wide usage.</p></div>
