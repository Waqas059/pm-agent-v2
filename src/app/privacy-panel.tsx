...
type DeletionPreview = {
  ...
  recentOperations: { id: string; status: "started" | "completed" | "failed"; started_at: string; completed_at: string | null; failure_reason: string | null }[];
};
...
export default function PrivacyPanel() {
  ...
  async function loadDeletionPreview() {
    ...
    setMessage("");
    try {
      const response = await authenticatedFetch("/api/workspace/delete");
  ...
  return <div>
    ...
    <div className="mt-4 rounded-xl border border-[#e3e7ee] bg-[#fafbfc] p-4 sm:p-5"><p className="text-xs font-bold uppercase tracking-[0.14em] text-[#8d98a9]">Workspace data controls</p><div className="mt-4 grid gap-3 sm:grid-cols-3"><a href="#context" className="min-h-11 rounded-lg border border-[#d8dee8] bg-white p-3 text-sm font-semibold text-[#526075] hover:border-[#bfc8d6]">Review or delete context<span className="mt-1 block text-xs font-normal text-[#8d98a9]">Manual records</span></a><a href="#documents" className="min-h-11 rounded-lg border border-[#d8dee8] bg-white p-3 text-sm font-semibold text-[#526075] hover:border-[#bfc8d6]">Review or delete files<span className="mt-1 block text-xs font-normal text-[#8d98a9]">Private documents</span></a><a href="#evidence" className="min-h-11 rounded-lg border border-[#d8dee8] bg-white p-3 text-sm font-semibold text-[#526075] hover:border-[#bfc8d6]">Review or delete evidence<span className="mt-1 block text-xs font-normal text-[#8d98a9]">Source-backed items</span></a></div><div className="mt-5 rounded-lg border border-[#f0d9cf] bg-[#fffaf7] p-4"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start"><div><h3 className="text-sm font-semibold text-[#192235]">Delete this workspace</h3><p className="mt-1 text-xs leading-5 text-[#8d98a9]">Owner-only, permanent deletion. Review the preview and type the exact confirmation text before anything is removed.</p></div><button type="button" onClick={() => void loadDeletionPreview()} disabled={isLoading} className="min-h-11 rounded-lg border border-[#e2b9aa] bg-white px-3 py-2 text-xs font-semibold text-[#a04c43] disabled:cursor-wait disabled:opacity-60">{isLoading && !preview ? "Loading…" : "Review deletion"}</button></div>{preview ? <div className="mt-4 space-y-3"><p className="text-xs text-[#526075]">This will remove <strong>{Object.values(preview.counts).reduce((total, count) => total + count, 0)} database records</strong> and <strong>{preview.storageObjectCount} private files</strong> from <strong>{preview.workspace.name}</strong>.</p><label className="grid gap-2 text-xs font-semibold text-[#526075]" htmlFor="workspace-delete-confirmation">Type <code className="rounded bg-white px-1.5 py-0.5 font-mono text-[#a04c43]">{preview.confirmationText}</code> to confirm<input id="workspace-delete-confirmation" value={confirmation} onChange={(event) => setConfirmation(event.target.value)} autoComplete="off" className="rounded-lg border border-[#d8dee8] bg-white px-3 py-2.5 text-sm font-normal text-[#192235] outline-none focus:border-[#a04c43] focus:ring-2 focus:ring-[#f0d4d0]" /></label><div className="flex flex-wrap gap-3"><button type="button" onClick={() => void deleteWorkspace()} disabled={isLoading || confirmation !== preview.confirmationText} className="min-h-11 rounded-lg bg-[#a04c43] px-3 py-2.5 text-xs font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50">{isLoading ? "Deleting…" : "Permanently delete workspace"}</button><button type="button" onClick={() => { setPreview(null); setConfirmation(""); setMessage(""); }} disabled={isLoading} className="min-h-11 rounded-lg border border-[#d8dee8] bg-white px-3 py-2.5 text-xs font-semibold text-[#526075]">Cancel</button></div>{preview.recentOperations.length > 0 && <div className="border-t border-[#eeddd8] pt-3"><p className="text-[11px] font-bold uppercase tracking-[0.12em] text-[#8d98a9]">Recent deletion attempts</p><ul className="mt-2 grid gap-2">{preview.recentOperations.map((operation) => <li key={operation.id} className="rounded-lg bg-white px-3 py-2 text-xs text-[#526075]"><div className="flex flex-wrap items-center justify-between gap-2"><strong className="capitalize">{operation.status}</strong><time dateTime={operation.started_at}>{new Date(operation.started_at).toLocaleString()}</time></div>{operation.failure_reason && <span className="mt-1 block text-[#a04c43]">{operation.failure_reason}</span>}</li>)}</ul></div>}</div> : null}{message ? <p className="mt-3 text-xs leading-5 text-[#a04c43]" role="alert">{message}</p> : null}</div><p className="mt-4 text-xs leading-5 text-[#8d98a9]">Automatic retention remains disabled. Records are retained by default until an authorized user deletes them.</p></div>
  </div>;
}
