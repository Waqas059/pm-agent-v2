...
export default function BetaAdminPage() {
  ...
  return <main className="beta-admin-page">
    ...
    {tab === "participants" && <section className="beta-admin-section">
      ...
      <div className="beta-admin-table-wrap"><table><caption className="sr-only">Beta participant engagement</caption><thead><tr><th>Name</th><th>Email</th><th>Country</th><th>Requests</th><th>Last active</th><th>Workflow progress</th><th>Useful output</th><th>Status</th><th /></tr></thead><tbody>{data.participants.map((participant) => <tr key={participant.id}><td><strong>{displayName(participant)}</strong><small>{participant.full_name !== displayName(participant) ? participant.full_name : ""}</small></td><td>{participant.email}</td><td>{participant.country_name || "—"}</td><td>{participant.used} / {participant.request_allowance}<small>{participant.remaining} remaining</small></td><td>{formatDate(participant.lastActive)}</td><td>{progressLabel(participant)}</td><td>{participant.usefulOutput ? "Yes" : "Not yet"}</td><td><span className={`beta-admin-status is-${participant.status}`}>{participant.status}</span></td><td><div className="beta-admin-row-actions"><button type="button" onClick={() => { setSelectedParticipant(participant); setAllowanceDraft(String(participant.request_allowance)); }}>Details</button><button type="button" onClick={() => void act({ action: "grant_allowance", id: participant.id, amount: 5 }).catch((error: unknown) => showError(error, "Could not grant allowance."))}>+5</button></div></td></tr>)}</tbody></table></div>
    </section>}

    {tab === "feedback" && <section className="beta-admin-section"><div className="beta-admin-feedback-list">{data.feedback.length ? data.feedback.map((item) => <article key={item.id}><div><strong>{item.usefulness_rating} / 5 · {item.would_use_again ? "Would use again" : "Would not use again"}</strong><small>{data.participants.find((participant) => participant.id === item.participant_id)?.email || "Participant"} · {formatDate(item.created_at)}</small></div><span className="beta-admin-status">{item.status}</span><p>{item.feedback_text || "No written note."}</p><button type="button" onClick={() => void act({ action: "review_feedback", id: item.id, status: item.wants_continued_access ? "follow_up" : "reviewed" }).catch((error: unknown) => showError(error, "Could not review feedback."))}>{item.status === "new" ? "Mark reviewed" : item.status}</button></article>) : <p className="beta-admin-empty">No beta feedback has arrived yet.</p>}</div></section>}

    {selectedParticipant && <div className="beta-admin-drawer-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setSelectedParticipant(null); }}><aside className="beta-admin-drawer" role="dialog" aria-modal="true" aria-labelledby="beta-participant-title"><div className="beta-admin-drawer-header"><div><p className="pm-eyebrow">PARTICIPANT DETAIL</p><h2 id="beta-participant-title">{displayName(selectedParticipant)}</h2><p>{selectedParticipant.email}</p></div><button type="button" className="beta-admin-close" aria-label="Close participant detail" onClick={() => setSelectedParticipant(null)}>×</button></div><dl className="beta-admin-detail-list"><div><dt>Status</dt><dd>{selectedParticipant.status}</dd></div><div><dt>Country</dt><dd>{selectedParticipant.country_name || "Not detected"}{selectedParticipant.country_code ? ` · ${selectedParticipant.country_code}` : ""}</dd></div><div><dt>First added</dt><dd>{formatDate(selectedParticipant.created_at)}</dd></div><div><dt>Last active</dt><dd>{formatDate(selectedParticipant.lastActive)}</dd></div><div><dt>Requests</dt><dd>{selectedParticipant.used} used · {selectedParticipant.remaining} remaining</dd></div><div><dt>Useful output</dt><dd>{selectedParticipant.usefulOutput ? "Yes" : "Not yet"}</dd></div><div><dt>Contact click</dt><dd>{selectedParticipant.contactClicked ? "Clicked" : "Not clicked"}</dd></div><div><dt>Feedback</dt><dd>{selectedParticipant.feedbackSubmitted ? "Submitted" : "Not submitted"}</dd></div></dl><div className="beta-admin-detail-section"><h3>Workflow progress</h3>{Object.entries(selectedParticipant.workflowProgress).map(([workflow, progress]) => <p key={workflow}><span>{workflow.replaceAll("_", " ")}</span><strong>{progress.completed} completed · {progress.started} started</strong></p>)}</div><div className="beta-admin-detail-section"><h3>Allowance</h3><div className="beta-admin-inline-form"><input aria-label="Participant allowance" type="number" min="0" value={allowanceDraft} onChange={(event) => setAllowanceDraft(event.target.value)} /><button type="button" onClick={() => void act({ action: "update_participant", id: selectedParticipant.id, allowance: Number(allowanceDraft) }).then(() => setMessage("Allowance updated.")).catch((error: unknown) => showError(error, "Could not update allowance."))}>Save allowance</button></div><p className="beta-admin-help">Use +5 for a small extension, or set a total allowance explicitly.</p></div></aside></div>}
  </main>;
}
